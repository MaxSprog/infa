...
int max(map<double, vector<int>> arr)
{
    int M = 0;
    for (map<double, vector<int>>::iterator it = arr.begin(); it != arr.end(); it++)
    {
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = i+1; j < n; j++)
        {
            ...
            if(crds[i][1] - crds[j][1] == 0){
                ...
                buf = (double)abs(crds[i][0] - crds[j][0]) / (double)abs(crds[i][1] - crds[j][1]);
            }
